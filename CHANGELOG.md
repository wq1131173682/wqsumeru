...
> **背景**：发现 AI 写作的重复模式（如"醒不了但能活"×N、对话同质化、结尾硬拔高）在写作阶段未做任何预防，全部堆到润色阶段才处理。本版本在 wq-write 的 context pack 生成流程中新增「批次反例扫描」步骤，写入前就拦截高频 AI 模式。

**改动**（1 个文件）：

- **`skills/wq-write/SKILL.md`**（v1.2.3→1.4.8）：父Agent 在子Agent写入**前**新增第 3 步「批次反例扫描」——扫描前 10 章提取高频 AI 模式，注入 `shared-write.md` 头部的 `⚠️ 本批次禁止项`。5 类检测规则：重复句式、结尾硬拔高、对话同质化、节奏单调、视角跳跃。无前文时（第 1-3 章）跳过。
- **向后兼容**：无前发现时不注入此节，行为与 v1.4.7 完全一致。
...
- 新增「九、反例警示注入」节：父Agent自检本批次AI特征（重复/视角跳跃/对话同质化/结尾硬拔高/节奏单调/副词堆砌），按 6 类规则注入禁止项
- 优先级：反例警示 > 标杆（禁止项是底线）
- 向后兼容：无AI特征时不注入
...
- 新增「八·五、执行协议」节，明确三处缺失：
  1. 子Agent必须写文件到 `.sumeru/polish/temp/{章号}.md`（不再只返回文本）
  2. 父Agent收集机制：检查temp→读取→字数验证→写回→清理
  3. 字数不足处理策略：≥90% 正常 / 70-90% 标记warning / <70% 禁止写回并回退
...
> **背景**：将技能仓库从原项目 fork 独立为自主品牌，所有技能前缀从 `sumeru-` 统一改为 `wq-`，同时更新项目名称、说明文档和目录结构。

**改动**（~30 个文件，涉及 470+ 处引用）：

- **A. 目录重命名**: 11 个技能目录 `skills/sumeru-*` → `skills/wq-*`
- **B. 内容替换**: 所有 `.md` 文件（排除 CHANGELOG.md）和 `.py` 脚本中的 `sumeru-` → `wq-` 引用替换，共 470+ 处
- **C. 项目名称更新**: "须弥写作 (Sumeru Writing)" → "WQ 写作 (WQ Writing)"
- **D. 说明文档同步**: QWEN.md、README.md、CLAUDE.md 标题和描述更新
- **E. 核心特性更新**: 模块计数 `10+2` → `11+1`（补入评分模块）
- **F. 保留原项目致谢**: 底部保留 `xindoo/sumeru` 开源项目归属

**不破坏项**：
- `.sumeru/` 项目数据目录路径不变（非技能前缀）
- `CHANGELOG.md` 历史记录保留原名称
- 所有 SKILL.md frontmatter `name` 字段已更新为 `wq-*`
- 所有技能内部交叉引用路径已更新
