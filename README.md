...
- **模块化架构**: 7个独立Skill模块（选题+大纲合并为 outline），可单独调用也可全流程自动编排
...
**润色级别说明：**
- 轻度：优化句式表达，去除冗余表述，精炼用词，提升文字流畅度（保留80%以上原文风格）
- 中度：重构段落结构，优化叙事视角，全面提升文笔质感（保留60%原文核心表达）
...
| 优化重点 | 优化重点 | 爽点强化、节奏收紧、对话优化、文笔提升、悬念增强 |
...
**功能**：错别字/标点/语法错误修正，三级敏感内容检测与修正建议，格式标准化，多平台格式导出，批量替换与自动分段，**批量处理时使用子Agent并行校验，每个Agent最多负责3个章节**。
...
| 参数 | 说明 | 示例 |
|------|------|------|
| 导出平台 | 导出平台格式 | 起点/番茄/晋江/纵横/17k/全平台 |
...
/sumeru-finalize 替换"张三"为"李玄" 替换"李四"为"王虎"       # 批量替换全文角色名称
