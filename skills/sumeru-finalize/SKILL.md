...
- **平台内容适配检查**（新增）：导出前执行全平台通用叙事效率检查，对标七猫/番茄免费阅读标准
...
| 检查项 | 阈值 | 严重度 | 说明 |
|--------|------|--------|------|
| 开篇前300字冲突启动 | ≤300字 | critical | 超限标记 `build_hook_delay` |
| 开篇前300字设定铺陈 | 0字 | high | 存在标记 `build_hook_expo` |
| 第1-3章结尾钩子 | 每章必须有 | critical | 缺失标记 `build_hook_missing` |
| 全局对话占比 | ≥25% | medium | 每章独立计算，汇总报告 |
| 全局内心独白占比 | ≤40% | medium | 每章独立计算，汇总报告 |
| 连续低密度警告 | ≤3章 | medium | 连续3章以上低密度标记 `build_density_low` |
| 章节字数范围 | 按 `project.json` 的 `chapterWordRange` | low | 标注超出范围的章节 |
...
| 叙事效率 | ⚠️ 18/30章 | 12章对话占比不足25% |
| 信息密度 | ⚠️ 前7章 | 连续7章低密度，每章仅1个核心事件 |

### 平台适配建议
- **七猫/番茄**：不推荐直接投递。开篇钩子不足 + 前7章节奏过慢
- **晋江**：可投递，建议优化对话占比后发布
...
| 阶段 | 负责内容 | 修改权限 |
|------|---------|---------|
| **sumeru-review** | 叙事效率 + 开篇钩子审查，输出 fix-plan | ✅ 可修改章节 |
| **sumeru-finalize** | Build前质量门禁，输出评分+平台适配建议 | ❌ 不修改章节，仅报告 |
