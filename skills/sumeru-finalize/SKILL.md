...
> 📌 **输出级别**：默认 `quiet` 模式，只输出进度和错误。详细规范见 `sumeru-rules` "输出级别规范"。
...
> - **子Agent规则**：待定项处理最多5个并行，每个最多20个待定项
> - **职责边界**：父Agent调用脚本做预处理，子Agent只处理待定项
> - **状态标记**：输出首行必须包含 `<!-- SUMERU_STATUS: batch=N, status=finalized, ... -->`
> - **Context Pack**：控制在1500-3000中文字，包含待定项列表、敏感词分级标准
