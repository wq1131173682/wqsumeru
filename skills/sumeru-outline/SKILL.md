...
| Context pack 生成 | ✅ 集中生成，含世界观/人物/分卷大纲摘要 | ❌ |
| 启动子Agent | ✅ 计算所需Agent数，按卷分配章节 | ❌ |
| **细纲生成** | ❌ | **✅ 唯一任务** |
| 合并细纲 | ✅ 汇总所有子Agent输出 | ❌ |
| 一致性校验 | ✅ 检查跨章衔接、人物连续性、时间线、跨卷伏笔 | ❌ |
| 写入 outlines/chapters.json | ✅ 统一写入，拆分生成 index.json + 分章JSON | ❌ |
| 刷新 cache | ✅ 统一刷新 project-brief、world-brief、character-brief、creative-brief | ❌ |

**调度逻辑**
```mermaid
flowchart LR
    A[细纲生成任务] --> B[父Agent: 确定总章数与分卷结构]
    B --> C[父Agent: 读取世界观/人物/分卷大纲]
    C --> D[父Agent: 生成 context pack & 按卷分配章节]
    D --> E[父Agent: 启动N个并行子agent]
    E --> F[子Agent: 读取 context pack → 生成细纲 → 输出细纲JSON/Markdown]
    F --> G{所有子Agent完成?}
    G -->|否| E
    G -->|是| H[父Agent: 合并细纲]
    H --> I[父Agent: 一致性校验: 跨章衔接/人物连续性/时间线/跨卷伏笔]
    I --> J[父Agent: 校验通过 → 写入 outlines/chapters.json]
    J --> K[父Agent: 拆分生成 index.json + 分章JSON & 刷新 cache]
```

**子Agent输入上下文（context pack）**：
- 已确定的世界观设定摘要（仅本组章节相关地点/组织/规则）
- 已确定的人物设定摘要（仅本组章节出场人物）
- 分卷大纲（本组章节所属卷的目标、核心冲突）
- 前序章节细纲摘要（用于情节衔接）
- 后续章节规划提示（用于伏笔规划）

**子Agent输出**：
- 每章细纲（JSON 或 Markdown 格式），包含：
  - 章节编号、标题、建议字数
  - 核心事件、出场人物、剧情推进步骤
  - 爽点/悬念、伏笔埋设、与前后章关联
  - 场景地点、POV视角、情绪基调
  - 章节目的、创意目标、规避套路、情绪节拍
  - 惊喜反转（如有）、输入/输出状态、验收标准
- **不写入任何文件、不更新任何状态**

**一致性校验（父Agent执行）**
- 合并所有子Agent生成的细纲后，执行一致性校验：
  - 检查相邻章节的情节衔接是否流畅
  - 检查人物出场连续性（人物状态、关系变化是否一致）
  - 检查时间线逻辑（事件时序是否合理）
  - 检查跨卷伏笔的设置与回收规划是否连贯
- 校验不通过时，标记冲突章节，由父Agent协调相关子Agent修正或手动调整
