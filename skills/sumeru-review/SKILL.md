...
- **平台适配验证**（新增）：开篇钩子强度、叙事效率、对话/叙述比例、章节信息密度，对标七猫/番茄免费阅读平台标准
...
仅对**前3章**执行。对标免费阅读平台读者留存规律：

| 指标 | 阈值 | 说明 |
|------|------|------|
| **前300字冲突启动** | 必须在300字内出现核心冲突/反常事件/悬念 | 超出→警告 `hook_delay` |
| **前300字无设定铺陈** | 前300字禁止出现世界观介绍、人物背景说明、环境长描写 | 出现→警告 `hook_expo_dump` |
...
| 指标 | 阈值 | 适用章节 | 说明 |
|------|------|----------|------|
| **对话占比** | ≥25% | 全部章节 | 对话占总字数比例。对白+引号内文本。不足→警告 `narrative_low_dialogue` |
| **内心独白占比** | ≤40% | 全部章节 | 心理描写/内心活动占总字数比例。超出→警告 `narrative_high_monologue` |
| **纯描写段落占比** | ≤35% | 全部章节 | 环境描写/外貌描写/动作细节描写。超出→警告 `narrative_high_description` |
| **每章核心事件数** | ≥1个 | 全部章节 | 每章至少有一个明确的情节推进事件。不足→警告 `narrative_low_event_density` |
| **相邻章核心事件重复** | 0次 | 全部章节 | 连续2章不能做同一件事（如"连续2章都在搭讪"属异常）。出现→警告 `narrative_event_stagnation` |

#### 信息密度检查

| 指标 | 阈值 | 说明 |
|------|------|------|
| **章内时间推进** | ≥1个时间节点变化 | 章内必须有至少一个时间推进（如"下午→傍晚"、"第2天"） |
| **章内地点变化** | ≥1个场景切换或地点变化 | 章内不能从头到尾待在同一个位置不变 |
...
### 开篇钩子
| 章节 | 前300字冲突 | 设定铺陈 | 结尾钩子 | 主角主动性 | 状态 |
|------|------------|---------|---------|-----------|------|
| 001 | ❌ 198字工作铺垫 | ⚠️ 展品清单背景 | ✅ | ✅ | hook_delay |
...
| 级别 | 条件 | 自动修复 | 处理方式 |
|------|------|---------|---------|
| **critical** | 前3章无结尾钩子 OR 连续5章以上低密度 | ❌ 不支持 | 写入 fix-plan.json，需重写 |
| **high** | 前300字有设定铺陈 OR 对话占比<15% OR 连续3章低密度 | ❌ 不支持 | 写入 fix-plan.json，建议重写 |
