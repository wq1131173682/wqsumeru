...
- 同一批次各章开场方式必须不同（禁止相邻两章都以"被叫醒""推门进来""新的一天"开头）
- 同一批次各章结尾钩子句式必须不同（禁止连续"XXX突然出现""XXX变了脸色"）
- 同一章内连续超过 5 句完整主谓宾结构 → 必须插入破碎句/口语短句打断节奏
```
...
- purpose: ...
- events: ...（≥3个具体事件）
- openingHook: 本章开场方式（可执行的场景描述，禁止"被叫醒"模板）
- acceptanceCriteria: ...
- creativeGoal: ...
...
## 开场与句式多样性提醒
- 各章开场方式必须不同，禁止连续两章以 "被叫醒" "推门进来" 等相同模式起始
- 各章结尾钩子句式不得雷同
```
...
**原则**：旧路径只读兼容，新写入一律使用 canonical 路径。
...
| `docs/glossary.md` | 只读引用，映射到 `plan.md` 术语表 | 用户手动整理 |
...
| 阶段 | 负责 Skill | 检查内容 | 输出 |
|------|-----------|----------|------|
| **选题阶段** | `sumeru-topic` | 风格-平台兼容性矩阵、章节字数-平台匹配 | 警告 + `platform-fit.json` |
| **审查阶段** | `sumeru-review` | 开篇钩子强度、叙事效率（对话/独白/描写占比）、信息密度 | 审查报告 + fix-plan |
| **构建阶段** | `sumeru-finalize` | Build 前质量门禁（同 review 指标，但不修改章节） | `build-quality-report.md` + 平台适配建议 |

**核心指标定义**（各阶段共用，定义于此，各 skill 引用）：
...
**平台风格兼容性矩阵**（定义于 `sumeru-topic`，其他 skill 引用时直接指向）：
- 详见 `sumeru-topic` 第三节"平台风格匹配规则"
