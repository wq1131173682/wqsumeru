...
> ⚠️ **重要**：这是给子Agent看的精简版规则。父Agent使用的是 `global-rules.md`。
...
如果父 Agent 在 context pack 中嵌入了用户风格特征（来自 `.sumeru/style-samples/user-style.md`），则：

1. 阅读用户风格特征，分析口语化程度、句式特点、对话风格、情绪表达、比喻风格
2. 主动模仿样本中的特征
3. 输出后自检：有没有"太正确"的地方？（如果有，对照样本修改）
4. 保留"奇怪"的部分，不要消除

**如果没有风格样本，跳过此节，按通用网文风格写作。**
