...
`sumeru-score` 设计为**可选的管道中间节点**，在 `polish → finalize` 之间插入：
...
1. **用户手动调用**：`/sumeru-score 全书 report`
...
```
/sumeru-score 全书 score-card           # 全书快速评分
/sumeru-score 全书 report               # 全书详细评分报告
/sumeru-score 卷1 score-card             # 仅评分第一卷
/sumeru-score 第1-50章 data             # 评分前50章，仅输出JSON
/sumeru-score 全书 report creativity    # 仅评分创意维度
/sumeru-score 全书 report completeness  # 仅评分完整性
```
...
| 上游 | 关系 |
|------|------|
| `sumeru-polish` | score 检测到 weak 维度 → 可触发 polish 定向修复 |
| `sumeru-finalize` | score 结果可作为 build 前门禁（S/A级允许发布，B级建议先polish，C/D级阻断） |
| `sumeru-review` | score 复用 review 的 continuity check / foreshadowing tracking 数据 |
| `sumeru-worldbuilder` | 编排时在 `polish → finalize` 之间可选择插入 score 阶段 |
