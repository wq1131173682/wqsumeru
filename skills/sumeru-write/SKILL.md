...
当 `.sumeru/project.json` 中 `chapterWordRange` 在 `[1000, 2000]` 范围内，或 `style` 字段包含"精品文"、"细腻"、"文艺"等关键词时，自动启用短篇精品文模式。

**模式调整：**

1. **章节节奏**：短篇不追求"每章一个爽点"，允许"平淡章"存在。每 3 章至少有一个情绪高点。
2. **情绪推进**：短篇的情感推进主要靠日常细节和对话，而非大事件。每一章的情绪节拍必须明确（如：平静→微澜→压抑→释放）。
3. **对话驱动**：短篇中对话承载主要情感推进，描写服务于对话，而非反过来。
4. **伏笔精简**：短篇伏笔不超过 5 条，且每条必须有明确的回收章节。
5. **结尾处理**：短篇不要求每章都有"钩子"，允许"留白式结尾"——不总结、不预告、只定格。
6. **字数控制**：严格遵循 `project.json` 中的 `chapterWordRange`，不超上限 20%。

**短篇精品文自检（替代爽文 5 条自检）：**

1. **情绪节拍**：本章是否有明确的情绪变化？（至少两个不同情绪状态）
2. **人物选择**：人物是否通过选择推动本章，而不是被事件推着走？
3. **字数达标**：是否达到 `project.json` 中 `chapterWordRange` 的下限？不够时在当前场景中自然展开——让人物多一个反应、多一句停顿、多一段沉默，而不是另加与场景无关的描写句。
4. **对话真实**：对话是否自然？（允许沉默、跑题、没说出口的话）
5. **结尾处理**：结尾是否既承接本章输出，又为下一章留空间？（不强制"钩子"）

以上 5 条全部通过才允许输出。任意一条不满足 → 重写调整后再提交。
...
#### 爽文题材额外检查（玄幻/仙侠/都市爽文等）
4. **爽点排重**：本章爽点是否和前 3 章重复？（同类型爽点隔 3 章再出现）
5. **反派智商**：反派或阻力是否足够聪明，是否让胜利付出代价？

#### 短篇精品文额外检查（言情/文艺/现实主义等）
4. **情绪节拍**：本章是否有明确的情绪变化？（至少两个不同情绪状态）
5. **字数达标**：是否达到 `project.json` 中 `chapterWordRange` 的下限？不够时在当前场景中自然展开。

以上检查中，通用 3 条必须全部通过。爽文额外 2 条或短篇额外 2 条根据题材二选一，全部通过才允许输出。任意一条不满足 → 重写调整后再提交。
...
| 自检 3 条通用 | 名场面/主动性/结尾 | 来自本文件 `创意落地自检` 节 |
| {题材自检项} | 根据题材动态替换 | 爽文→"爽点排重+反派智商"；短篇精品→"情绪节拍+字数达标" |

**遵守规则：**
- 题材必须精确（如"玄幻仙侠"），不能笼统写"网文"
- 风格描述直接取自 `反AI写作规则` 的人类式列，不自行修改措辞
- 通用自检 3 条逐字使用，不删减、不合并
- {题材自检项} 根据 `.sumeru/project.json` 中的 `style` 字段动态替换：
  - 包含"爽文""热血""快节奏" → "爽点排重+反派智商"
  - 包含"精品文""细腻""文艺" → "情绪节拍+字数达标"
