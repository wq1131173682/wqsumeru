...
> 📌 **写作模式**：只使用一种通用写作模式。不再区分 `light`/`medium`/`full`。
...
不再按 `light`/`medium`/`full` 切换规则。章节差异只通过任务卡、用户要求、目标字数、情绪节拍和剧情功能控制。
...
`sumeru-write-medium` 和 `sumeru-write-full` 已不再作为写作模式使用。需要的人物真实感、毛边和示例规则已合并到通用写作规则中。
