...
> **核心原则：先确保好看，再确保不像AI。好看但像AI > 不像AI但不好看。**

子Agent写作时先通过以下读者吸引力检查，再执行反AI规则：

| 自检项 | 判定标准 | 不通过表现 |
|--------|----------|-----------|
| **代入感** | 前 300 字能让读者代入主角处境或情绪吗？ | 开篇大段设定说明、心理独白缺乏情境锚点 |
| **期待感** | 读完后读者会想知道接下来发生什么吗？ | 章末无悬念、无未解问题、无情绪余味 |
| **情绪共振** | 本章至少有一个情绪点能击中读者吗？ | 全程信息通报式叙述，无情绪起伏 |
| **记忆点** | 本章有一句/一幕读者看完能记住的吗？ | 全章无独特画面、无金句、无意外转折 |

**4 条全部通过才允许进入反AI自检。** 任意一条不通过 → 找到薄弱点强化后重写。

### 反AI写作规则（写作原则，润色时再精细调整）

> **核心原则：AI 写得越"正确"的部分越要改，写得越"奇怪"的部分越要保留。**
> **优先级：读者吸引力自检 > 反AI规则。不能为"像人类"牺牲"好看度"。**
...
子Agent写入章节前逐条检查。**自检分为两层：读者吸引力自检（通用，必须先通过）→ 创意落地自检（按模式）**。

#### 第一层：读者吸引力自检（所有模式通用，优先执行）
1. **代入感**：前 300 字能让读者代入主角处境或情绪吗？
2. **期待感**：读完后读者会想知道接下来发生什么吗？
3. **情绪共振**：本章至少有一个情绪点能击中读者吗？
4. **记忆点**：本章有一句/一幕读者看完能记住的吗？

4 条全部通过后进入第二层。

#### 第二层：模式自检

根据 `.sumeru/project.json` 的 `style` 字段自动选择：
...
两层共计 9 条全部通过才允许输出。任意一条不满足 → 重写调整后再提交。
...
- 自检分两层共 9 条：第1-4条为读者吸引力（通用），第5-9条根据 `style` 字段动态替换：
  - 爽文模式：名场面/主角主动性/结尾钩子/爽点排重/反派智商
  - 精品文模式：名场面/主角主动性/字数达标/情绪节拍/结尾处理
...
- **情绪继承**（前3章情绪曲线、当前读者预期、需避免的重复情绪、本章情绪任务）
- Creative Strategy（创意目标、情绪节拍、要避开的套路）
- Creative Anchors（每章必须体现至少1个锚点）
- Batch Summary（非第一批时）
...
父Agent在生成 `shared-write.md` 时，必须包含以下情绪继承块：

```markdown
## 情绪继承
- 前3章情绪走向：[紧张→释放] → [舒缓→不安] → [压抑→爆发]
- 前3章爽点类型：第N章-打脸，第N+1章-升级，第N+2章-收获
- 当前读者预期：期待主角反击（已铺垫3章）/ 期待揭秘真相 / 期待关系推进
- 需避免的重复：不要再出现"被打压"爽点，读者耐心已到临界点
- 本章情绪任务：必须给出一次有效的反击/收获作为情绪释放
- 已用情绪模板：第N章-A型(反转型)，第N+1章-C型(抑扬型)，第N+2章-E型(悬念型)
- 本轮可用模板：B/D/F/G（排除已用3章内的A/C/E）
```

**规则**：
- 情绪走向和爽点类型速查不依赖完整摘要，父Agent从最近3章的 SUMERU_STATUS state_diff 中解析
- "当前读者预期"由父Agent根据前3章累积的未解决张力判断
- "本章情绪任务"是父Agent对本章"必须给读者什么情绪释放"的指令
