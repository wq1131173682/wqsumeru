...
**风格样本不是强制要求**。如果用户想让自己的写作风格被 AI 模仿，可以在写作过程中提供。

#### 使用方式

1. **主动提供**：用户随时可以告诉 AI "我想提供写作风格样本"，然后粘贴 4 段写作片段（对话/情绪/叙事/比喻），AI 会分析并生成风格特征，后续写作时自动模仿
2. **写作时询问**：首次写作时，父 Agent 可以询问"是否提供写作风格样本？（可选，提供后 AI 会模仿你的文风）"
...
| 样本 | 采集目标 | 字数 |
|------|----------|------|
| 对话风格 | 对话节奏、口头禅、停顿改口 | 300-500字 |
| 情绪描写 | 直接说情绪 vs 动作承载、情绪矛盾 | 300-500字 |
| 叙事节奏 | 冗余程度、逻辑跳跃、留白 | 300-500字 |
| 比喻/意象 | 个人化程度、精准度、类型偏好 | 300-500字 |

#### 风格特征注入

如果用户提供了样本，父 Agent 会：
1. 分析样本，提取用词习惯、句式特点、对话风格、情绪表达、比喻风格
2. 将风格特征嵌入后续写作的 context pack
3. 子 Agent 写作时主动模仿，输出后自检"有没有太正确的地方"

#### 项目级存储

风格样本文件存放在项目目录 `.sumeru/style-samples/`：
