...
3. **字数达标**：是否达到 `project.json` 中 `chapterWordRange` 的下限？不够时在当前场景中自然展开。
...
1. 本章有没有读者能记住的一幕？
2. 主角是主动选择还是被剧情推着走？
3. 字数是否达到 project.json 中 chapterWordRange 的下限？
4. 对话是否自然？
5. 结尾是否既承接本章输出，又为下一章留空间？
```
...
| 5 条自检 | 名场面/主动性/字数/对话/结尾 | 来自本文件 `创意落地自检` 节 |
