...
规整项目、迁移旧项目、补齐缺失文件、检查项目完整性、修复项目结构、升级项目格式、旧项目升级、项目检查、查缺补漏、项目规范化

### 核心功能

1. **项目扫描**：全面扫描现有项目结构，识别文件缺失和字段问题
2. **路径迁移**：旧路径自动迁移到新 canonical 路径
3. **配置补齐**：生成缺失的 `.sumeru/project.json`、`status.json` 等配置文件
4. **目录补齐**：创建缺失的目录结构
5. **字段补全**：补齐章节任务卡缺失的必填字段
6. **人物卡生成**：从大纲中提取人物信息生成人物卡
7. **迁移报告**：生成详细的迁移报告
...
> **目的**：检查 `wq-worldbuilder` 恢复所需的前置文件是否齐全。
...
> **详细处理规则**：见「第十部分·分卷迁移」与 `wq-rules/SKILL.md` 第十五部分·分卷隔离与卷切换协议。

| 检查项 | 修复方式 |
|--------|----------|
| 扁平项目但 chapters ≥ 150 | 仅提示用户「建议升级为分卷模式」，不强制执行；用户需主动调用 `/wq-migrate 分卷迁移` 才会进入第十部分 |
...
- **前置**：无（可直接调用：- **后续**：迁移完成后可继续使生`wq-worldbuilder` 恢复创作，或使用其他 Skill 继续
...
> **具体规范**：分卷数据模型、目录结构、激活/降级规则详见 `wq-rules/SKILL.md` 第十五部分·分卷隔离与卷切换协议；本节仅描述迁移阶段的执行流程。
...
| 触发方式 | 说明 |
|----------|------|
| 显式调用 `/wq-migrate 分卷迁移` | 用户主动请求升级为分卷模式 |
| 显式调用 `/wq-migrate 启用分卷` | 同上，语义别名 |
...
**优先级规则**（在 wq-write / wq-review 读取时）：
...
- **不修改** `status.json` 的 `chapterStatus` 字段（分卷后由 wq-worldbuilder 在卷切换时自动管理 `vol-N/status.json`）
...
> **目标**：确保 `wq-migrate` 完成后，项目处于"可被 `wq-worldbuilder` 恢复"的就绪状态。
> **适用版本**：v1.3.0 起。
>
...
完整迁移（`/wq-migrate` 不带子参数）必须在「生成报告」之后依次执行以下步骤，缺一不可：
...
| **H4** | `.sumeru/context-packs/` 是否存在（仅 long/full 模式） | 按 `wq-rules` 第七部分 Schema 初始化空目录 |
...
## 确认提示
请用 `/wq-worldbuilder {原题材} 锚点确认` 逐条确认、修改或新增，确认后状态从 `pending` 改为 `confirmed` / `user_modified` / `user_added`。
```
...
```json
{
  "migrationHandoff": {
    ...
    "fromVersion": "1.0.0",
    "migratedBy": "wq-migrate",
    "anchorStatus": "pending",
    "introStatus": "pending",
    "continuityRebuilt": true,
    "recommendedNext": "/wq-worldbuilder 恢复上次创作"
  }
}
```
...
`/wq-worldbuilder 恢复上次创作` 检测迁移 handoff 的逻辑：
...
4. 如果不存在：
   ...
   b. 如果存在 → 提示用户"建议先运行 /wq-migrate 完成接续协议"
