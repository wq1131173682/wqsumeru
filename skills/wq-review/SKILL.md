...
3. 若缺少`outlines/chapters.json`，按旧路径兼容策略处理（见`wq-rules/SKILL.md` 第六部分"独立调用自举协议"）
...
- 重写修复 → 生成 `fix-plan.json`，由 `wq-worldbuilder` 编排或用户手动调用`wq-write` 处理
...
| **标点错误** | 规则替换 | 连续逗号"，，"改为"、"、英文标点改为中文标点、省略号格式统一为"……"、破折号格式统一为"——"、引号/书名号/括号闭合检查、标点前后空格清理（详见 `wq-rules` 第六点五部分） |
...
| **字数不足** | ⚠️ **不再自动注入描写段**（防止水文）。改用反 AI 扫描+ fix-plan 标记，触发 write 重写 | 调用 `python skills/wq-review/scripts/anti-ai-scan.py <chapters_dir>` 报告问题；将 `word_count_short` 写入 `fix-plan.json`，由 write 阶段在当前场景中"自然展开"（让人物多一个反应、多一句停顿、多一段沉默）补足 |
...
2. 调用 wq-write 走重写流程，强制 context pack 中携带反 AI 扫描报告
...
> **核心指标阈值定义**见`wq-rules/SKILL.md` 第十三部分"平台适配规则索引"。本节仅定义审查阶段的执行流程和输出格式。
#### 开篇钩子强度检查
...
> **适用条件**：项目 `project.json.volumeCount >= 2`（详见 `wq-rules/SKILL.md` 第十五部分·分卷隔离与卷切换协议）。未启用分卷模式的项目继续使用扁平 continuity 路径，本节不生效。
...
- 详细规范见 `wq-rules/SKILL.md` 第十五部分·分卷隔离与卷切换协议
### 与其他Skill 配合
- **前置**：`wq-write` 生成的`chapters/` 和`wq-outline` 的大纲数据
- **后续**：输出供 `wq-polish`、`wq-finalize` 使用
