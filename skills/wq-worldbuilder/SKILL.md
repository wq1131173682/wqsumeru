...
> **与 wq-write 的区别**：worldbuilder 负责"从零到完稿"的全流程统筹；write 负责具体的写续写/扩写/重写章节。用户说"帮我写小说且未指定具体章节时走 worldbuilder；明确说"写第X章、续写"、扩写这段"时走 write。

### 核心功能
worldbuilder 是网文创作的一站式主控技能，负责统筹协调从创意萌芽到作品完稿的完整创作链路：

0. **项目初始化**：创建标准小说项目目录
1. **选题策划**：调用 `wq-topic` 进行市场分析、选题定位、创意引擎、平台定向
2. **大纲设计**：调用 `wq-outline` 构建完整世界观、人物设定、分卷大纲与章节任务单
...
3. **内容创作**：调用 `wq-write` 按章节任务卡进行分章节内容撰写
4. **逻辑审查**：调用 `wq-review` 对已完成章节进行项目测试式审查
...
3. **分卷模式判定**：当以下任一条件满足时启用分卷模式 —
   ...
   满足条件时计算 `volumeCount`：按每卷 150-250 章均分计划章节数，记录到 `.sumeru/project.json` 的 `volumeCount` / `currentVolume` / `volumes[vol-N]` 字段。例如 200 章 / 每卷约 150 章 → `volumeCount = 2`：`vol-001`（第 1-150 章）、`vol-002`（第 151-200 章）。结果写入 `.sumeru/decisions.md`。具体规范见 `wq-rules/SKILL.md` 第十五部分·分卷隔离与卷切换协议。
...
>
> **配套**：`wq-migrate` v1.3.0+ 写入 `status.json.migrationHandoff` 字段；本端读取该字段决定续作路径。
> **字段 Schema**：`wq-migrate/SKILL.md` §11.5、**状态机语义**：`wq-rules` §状态字段语义对照。

#### 触发命令
```
/wq-worldbuilder {题材} 恢复上次创作
```
...
| `migrationHandoff` 不存在但 `.sumeru/migration.json` 存在 | **警告**：旧版迁移未出 handoff。提示先跑 `/wq-migrate` 补齐接续协议（子模式选"补齐配置"），再回来续作 |
...
#### recommendedNext 字段
- 读 `recommendedNext`，若为 `/wq-worldbuilder 恢复上次创作`，按上表三态处理
- 若为 `/wq-outline 重新生成` 等其他值，原样输出推荐命令给用户，**不替用户做决定**
- 字段缺失时默认 `"/wq-worldbuilder 恢复上次创作"`，按 handoff 是否存在继续走三态分支
...
**用户可指定输出级别：**
```
/wq-worldbuilder 玄幻 "废柴逆袭" 输出级别 normal
/wq-worldbuilder 玄幻 "废柴逆袭" 输出级别 verbose  # 调试用
```
...
### 使用示例
```
/wq-worldbuilder 玄幻 "废柴逆袭+系统流"
/wq-worldbuilder 言情 "霸道总裁+契约恋爱" 标题 "总裁的契约新娘"
/wq-worldbuilder 玄幻 "废柴逆袭+系统流" 恢复上次创作
/wq-worldbuilder 都市 "职场+重生" 跳过选题阶段
/wq-worldbuilder 悬疑 "连环杀人心理侧写+反转" 详写风格 暗黑调
```
...
| 字段 | 适用类型 | 必填 | 含义 |
|------|----------|------|------|
| `targetChapter` | 名场面种子 | 否 | 预期兑现章节号；wq-write 写前检查：距当前章 ≤ 3 时在 context pack 头部注入"请在本章或近期兑现 XX 锚点"提醒 |
| `protectedTag` | 全部 | 否 | 锚点保护标签，被 wq-write 子 Agent 在 `protectedElements` 字段中引用；多个锚点共享同一 tag 表示绑定兑现 |
...
>
> **完整规范**：见 `wq-rules/SKILL.md` 第十五部分·分卷隔离与卷切换协议。本节只描述 worldbuilder 的编排职责与状态机衔接。
...
1. 检查当前卷所有章节状态：若存在非 `drafted`（或更高）章节 → 调用 `wq-write` 补写至至少 `drafted`
