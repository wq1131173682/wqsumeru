...
> **与 wq-worldbuilder 的区别**：write 负责具体的章节写作操作（写一章、续写、扩写、重写、批量生成）。用户说"帮我写小说"且未指定具体章节时走 worldbuilder 统筹；明确说"写第X章"、"续写"、"扩写这段"时走 write。

### 核心功能
1. **基于完整细纲生成**：自动读取`outlines/chapters.json`（旧路径兼容见`wq-rules/SKILL.md` 第六部分"独立调用自举协议"）
...
2. 若缺少`outlines/chapters.json`，按旧路径兼容策略处理（见`wq-rules/SKILL.md` 第六部分"独立调用自举协议"）
...
- **标点符号是否规范？** 检查破折号滥用（`——`只用于中断/意外/突转）、省略号格式（用`……`不用`...`）、感叹号叠用（`！！！`→`！`）、中英文标点混排（详见 `wq-rules` 第六点五部分）→ 按规范修正
...
```
⚠️ 文件写入路径（必须严格使用，禁止自行推断）：
- 第X章 → 写入 `chapters/001-标题.md`
- 第Y章 → 写入 `chapters/002-标题.md`
- 第Z章 → 写入 `chapters/003-标题.md`
（文件名格式：`{三位章节号}-{章节标题}.md`，标题从任务卡的 title 字段取）
...
> **详见 `wq-rules/SKILL.md` 第四部分"Context Pack 格式"与第五部分"分卷数据源作用域规则"。** 完整目录结构与卷切换协议见 `wq-rules/SKILL.md` **第十五部分·分卷隔离与卷切换协议**。
...
当 `.sumeru/research/writing-guide.md` 存在时（由 `wq-scan` 生成），父Agent在生成 context pack 时必须：
...
当用户指定仿写目标时（如 `/wq-write 第1章 --imitate 《书名》`），父Agent执行以下流程：
...
```bash
# 指定仿写目标
/wq-write 第1章 --imitate 《书名》

# 指定仿写指南文件
/wq-write 第1章 --imitation-guide imitation-guide-XX.md

# 批量仿写
/wq-write 第1-10章 --imitate 《书名》
```
...
5. **（可选）连续性冲突检查**：若 `consistency-rules.json` 已有累积状态，调用 `python skills/wq-review/scripts/continuity-check.py .sumeru/continuity --quiet`，把 critical 冲突立即报出；high/medium 写入 `.sumeru/issues.md`。**分卷模式**：脚本输入路径替换为 `.sumeru/volumes/vol-N/continuity/`，父 Agent 调用前先 `export SUMERU_CURRENT_VOLUME=vol-N`；跨卷冲突写入 `.sumeru/cross-volume/dependency-table.md` 的「已发现冲突」section。
 6. **截断检测（v1.4.9 新增）**：检查子 Agent 输出是否截断（详见 `wq-rules/SKILL.md` 第七部分）。疑似截断 → 触发续写协议（最多 1 次），合并后写入 `chapters/`。
...
### 细纲驱动批量生成
```
/wq-write 全部章节       # 生成所有章节（自动并行）
/wq-write 第1-50章      # 生成指定范围
/wq-write 第3章         # 生成特定章
/wq-write 第3章 "概要"   # 单章创作
/wq-write 第3章 按细纲生成
/wq-write 第1卷         # 分卷模式：生成第1卷所有章节（详见「分卷模式支持·四」）
/wq-write vol-003       # 用 vol-ID 形式指定卷
```
...
> 完整目录结构与卷切换协议见 `wq-rules/SKILL.md` **第十五部分·分卷隔离与卷切换协议**。
...
python skills/wq-review/scripts/foreshadowing-tracker.py \
...
新增 `/wq-write 第N卷` 调用形式，父 Agent 自动按卷范围批量生成该卷所有章节：

**触发命令示例**：
```bash
/wq-write 第1卷               # 写第1卷所有章节
/wq-write 第2卷 仙侠风格       # 第2卷 + 风格覆盖
/wq-write vol-003              # 用 vol-ID 形式
/wq-write 第1卷 --from-batch 5 # 从第5批开始（卷内偏移）
```
...
4. 卷内批次正常串行（与扁平模式相同）
5. 卷尾（最后一批次）若为该卷最后章节，**仅在用户明确要求**时触发 `wq-write` 父 Agent 的 `Phase A` 卷完结流程（最终快照 + 卷级总结 + 跨卷骨架更新）；**默认不自动**触发 Phase A，留给 `wq-worldbuilder` 编排

**章节范围约定**：`/wq-write 第1卷` 与 `/wq-write 第1-30章`（章节号范围）可以混用；优先按 `volume` 字段匹配，未匹配时回退到范围语法。
...
**分卷模式（`project.json.volumeCount >= 2`，详见「分卷模式支持」与 `wq-rules/SKILL.md` 第十五部分）：**
...
### 与其他Skill 配合
- **前置**：`wq-topic` 选题 + `wq-outline` 大纲（`plan.md`、`outline.md`、`outlines/chapters.json`）
- **前置（可选）**：`wq-scan` 扫榜分析（`writing-guide.md` 注入context pack）
- **前置（可选）**：`wq-scan` 仿写指南（`imitation-guide-{书名}.md` 注入context pack）
- **后续**：供 `wq-review`、`wq-polish`、`wq-finalize` 使用

> 风格样本机制详见 `wq-rules/SKILL.md` 第十二部分子Agent精简版规则中的风格样本说明。用户提供样本后父Agent注入 context pack。
> 反AI写作规则详见本文`反AI写作规则` 节和 `人物真实感与毛边规则` 节。
> 分卷模式完整规范（目录结构、卷切换协议、Phase A/B/C、激活与降级）见 `wq-rules/SKILL.md` **第十五部分·分卷隔离与卷切换协议**；本文「分卷模式支持」节是 write 侧的操作约束。
